...
const task = defineTask({
  ...
  run: async (payload, ctx) => {
    ...
    console.info(`[task ${ctx.publicId}] ECHO_FROM_EXPRESS firing fetches`, { sender: payload.sender });
    const body = JSON.stringify({ from: "tasks-owner", sender: payload.sender, taskId: ctx.publicId });
    const headers = { "content-type": "application/json" };
    const expressTarget = `${EXPRESS_URL}/echo`;
    const nestTarget = `${NESTJS_URL}/from-express`;
    await Promise.all([
